...
Uchoice = [-1,-1,-1,-1,-1]; // this array holds the users choices
Gchoice = [0,2,1,-1,-1,-1]; // this array is the good choice for each stage
Bchoice = [2,0,0,-1,-1,-1]; // this array is the bad choice for each stage
...
function choice(choice) {
    if (i <= 2) {
        if (choice == 'fight' || choice == 'Fight') {
            ...
            Uchoice[i] = 1;
        } else if (choice == 'hide' || choice == 'Hide') {
            Uchoice[i] = 2;
        } else {
...
function get_input() {
  Uchoice[i] = choice(document.getElementById('input').value)
}

function Restart() {
  document.getElementById('Story').innerHTML = next[0];
